...
/*删除admin库的书*/void book_out(int out_id)
{
	...
	while (fscanf(file, "%d\t%s\t%s\t%d", &id, title, author, &year) == 4) {
		...
		else if (id == out_id)correct++;
	}
		if (correct == 0) {
			system("cls");
			printf("没有这本书\n");
			name = fopen("temp.txt", "r");
			fgets(role, sizeof(role), name);
			role[strcspn(role, "\n")] = '\0';
			if (strcmp(role, "admin") != 0) all_core('1');
			else all_core('3');
		}//避免凭空借书
	fclose(file);fclose(temp);//不关闭不保存
	if (remove("admin") != 0) {
		printf("删除文件失败！\n");
		return;
	}
...
{
	FILE* file1 = fopen("admin", "r"); // 打开文件以只读模式
	if (file1 == NULL) {
		perror("图书馆没书辣");
	}
	int id;             
	...
	int year;
	printf("%-10s%-30s%-30s%-10s\n", "id", "title", "author", "year");
	while ( fscanf(file1, "%d\t%s\t%s\t%d", &id, title,author, &year) == 4) {//临时抽取4个作为检查,每次file会指向下一个
		//id[strcspn(id, "\n")] = '\0';
		title[strcspn(title, "\n")] = '\0';
...
again_choose:
	...
	if (role==1) {//user
		...
		case '2': link = creatlink(); book_in(link); system("cls"); printf("已成功借阅如下图书\n"); printlink(link); all_core('1'); break;//借书，避免了凭空借
...
/*梦开始的地方*/void start()
{
	...
	all_core(choose);
	while (1);//没意义的备注很累，比如说这个是为了调试不退出;
	//这个函数是为了返回上一页而作，前面试了嵌套，返回值总是第一个选择，存最后一个用文件存有点麻烦，用全局变量怕乱掉
